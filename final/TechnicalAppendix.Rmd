...
```{r Mexico Data, echo=TRUE}
MexicoResidentialData <- EuropeAmericaDataPostFeb6 %>% filter(country %in% c("Mexico"))

MexicoResPlot <- ggplot(MexicoResidentialData, aes(x = date, y = gcmr_residential)) + geom_smooth(size = 0.5) + xlab("Date") + ylab("Activity in places of residence") + ggtitle("Activity in places of residence against the date in the Mexico")
print(MexicoResPlot)
...
MexicoTakeoutPlot <- ggplot(MexicoTakeoutSearch, aes(x = date, y = Take.out...Mexico.)) + geom_smooth(size = 0.5) + xlab("Date") + ylab("Interest in 'Take-out' searches") + ggtitle("Interest in 'Take-out' against the date in the Mexico")
...
```{r MexicoTakeoutResPlot, echo=TRUE}
MexicoTakeoutResPlot <- ggplot(MexicoTakeoutResData, aes(x = gcmr_residential, y = Take.out...Mexico.)) + geom_point(size = 0.5) + geom_smooth()+ ylab("Interest in 'Take out' searches over time ") + xlab("Activity in places of residence\n(as a percentage*100 change relative to the baseline period)") + ggtitle("Interest in 'Take out' against the time spent in places of\nresidence in Mexico") + ylim(c(0,100)) 
...
Similar to the UK, the government of Mexico implemented 2 lockdown measures in late March. This suppressed spread of the COVID-19 virus until July when the daily confirmed cases was back on the rise. As the number of confirmed cases increased the activity in places of residence followed. We can also observe an increase in the interest for "Take out".
