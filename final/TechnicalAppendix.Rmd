 ...
 * Lockdown decreased significantly physical retail and recreational activity, leading to ecommerce boom.

# Analysis
## Confirmed cases

In the following code chunks, we will be extracting the daily confirmed cases of COVID-19. We will also be plotting the number of lockdown measures against the date, and having a look into the COVID-19 cases over time.
...
plot1 <- {glob_mob %>% ggplot(aes(x=date,     
                                  ...
                                  color=region))} +
  geom_smooth() +
  scale_colour_discrete(labels = c('Europe',"North America",'South America')) +
  xlab("Date") +
  ...
  guides(color=guide_legend("Region")) +ggtitle("Substantial decrease in visits to grocery shops and pharmacies after first lockdown",subtitle = "Drawn from Google Community Mobility Reports")
print(plot1)
```
...
USTakeoutResPlot <- ggplot(USTakeoutResData, aes(x = gcmr_residential, y = Take.out...United.States.)) + geom_point(size = 0.5) + geom_smooth()+ ylab("Interest in 'Take out' searches over time ") + xlab("Activity in places of residence\n(as a percentage*100 change relative to the baseline period)") + ggtitle("Interest in 'Take out' against the time spent in places of\nresidence in the US") + ylim(c(0,100))
